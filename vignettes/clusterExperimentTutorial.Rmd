...
Below we will describe more about the options we could adjust in `RSEC`. But we might, for example, want to change the proportion of co-clustering (which was the default of 0.7) we required in making our `combineMany`
 clustering, or change the proportion of genes that must show differences in order to not merge clusters or the method of deciding. We can call `RSEC` again on our object `rsecFluidigm` and it will not redo the many clustering steps (unless we request it to rerun it by including the argument `rerunClusterMany=TRUE`). We set the proportion of co-clustering required by the argument `combineProportion=0.6`, we make the merge cutoff `mergeCutoff=0.01`, and we decide to use a different method of estimating the proportion differential for merge by setting `mergeMethod="Storey"` instead of the default ("adjP"). These are the main parameters we might want to tweak in these steps.
