...
Note we set a large number of parameters here to make sure that the vignette is back-compatible in case defaults change, we will explain these parameters as we go along, but many of these parameters are the default. 

```{r RSEC, eval=FALSE}
library(clusterExperiment)
system.time(rsecFluidigm<-RSEC(se, isCount = TRUE, 
	reduceMethod="PCA", nReducedDims=10,consensusMinSize=3,
	ncores=1, minSizse=1, clusterFunction="hierarchical01",
	mergeCutoff=0.05,consensusProportion=0.7,
	random.seed=176201))
```
...
Note that this is not the result from any particular subsampling (which was done repeatedly for each clustering), but the proportion of times across the clusterings we ran. The initial consensus clustering in `makeConsensus` was made based on these proportions and a particular cutoff of the required proportion of times the samples needed to be together.


**Plot of Dendrogram** We can visualize how the initial ensembl cluster in `makeConsensus` was clustered into a hierarchy and merged to give us the final clustering in `mergeClusters`: 
...
plotCMar<-c(1.1,8.1,4.1,1.1)
par(mar=plotCMar)
plotClusters(rsecFluidigm,main="Clusters from RSEC", whichClusters=c("mergeClusters.1","makeConsensus.1","mergeClusters","makeConsensus"), sampleData=c("Biological_Condition","Published2"), axisLine=-1)
```
...
rsecFluidigm<-RSEC(rsecFluidigm,consensusMinSize=3)
par(mar=plotCMar)
plotClusters(rsecFluidigm,main="Clusters from RSEC", whichClusters=c("mergeClusters","makeConsensus"), sampleData=c("Biological_Condition","Published2"), axisLine=-1)
```

```{r results="hide",echo=FALSE}
# a small check to make sure have same clusterings as before
if(!all(primaryCluster(rsecFluidigm)==clusterMatrix(rsecFluidigm)[,"mergeClusters.2"])) stop("rerunning rsec has changed clustering")
```
...
** Additional clusters ** If the option is `plotType="colorblock"` and `leafType="samples"` (i.e. the default) we can also vary the sample information that is plotted with the dendrogram, much like with `plotHeatmap`. In particular, we can show multiple clusters using the option `whichClusters`.

```{r plotDendroMultiClusters}
plotDendrogram(ce,whichClusters=c("makeConsensus","mergeClusters"))
```


** Merge Information** We can also plot the merging information, with `merge="mergeMethod"`, i.e. the method actually used to merge the data. We could alternatively plot any merge method that has been run from a call to `mergeClusters`. 

```{r plotMerge}
plotDendrogram(ce,whichClusters=c("makeConsensus","mergeClusters"),sampleData=c("Biological_Condition", "Published1","Published2"),merge="mergeMethod")
```

** Node information ** Finally, we can also color code the nodes or plot the node names, for example if we want to link them to our `getBestFeatures` results.
...
### Example using a user-defined clustering function

Here, we show how to use a user-defined clustering function in `clusterSingle`.
...
SNN_wrap <- function(x, k, steps = 4, ...) {
  snn <- scran::buildSNNGraph(x, k = k, d = NA, transposed = FALSE)
  res <- igraph::cluster_walktrap(snn, steps = steps)
  return(res$membership)
}
```

Our wrapper has two required parameters: `x`, a matrix with the data to be clustered; and `k` an integer value that specifies the number of nearest neighbors. The function returns a integer vector with the cluster labels.

`clusterExperiment` provides the function `internalFunctionCheck` that validates user-defined cluster functions. Among other things, it checks that the input and output are compatible with the `clusterExperiment` workflow. Please, see `?internalFunctionCheck` for details.
...
If it passes all checks, we can then create an object of the S4 class `ClusterFunction` to be used within the package.
This contains, in addition to the function definition, information of the type of input and output expected by the function. This information is passed to `clusterMany` and `clusterSingle` so that they know what to pass and what to expect from the user-defined method.
...
Now that we have our `SNN` object, we can treat our custom method as a base clustering routine to be used in `clusterMany`, similarly to how we used `kmeans` and `pam` earlier.

In this example, we use `clusterSingle` to implement a subsample clustering based on SNN.
...
```{r custom_clusterMany, eval=FALSE, include=FALSE}
...
We can also manually choose the set of clusters to use in `makeConsensus` with the argument `whichClusters`. Here we choose only the clusters that correspond to using dimensionality reduction using the most variable features. We also set `minSize` to be lower than the default of 5 to allow for smaller clusters
...
### Consensus of subsets of clusterings
...
`clusterExperiment` makes it easy to extract a subset of clusterings and to compute a consensus clustering for any given subset, to help addressing this type of questions.
...
```{r sub_makeConsensus}
clusterNames15 <- rownames(params)[which(params$nReducedDims == 15)]
clusterNames50 <- rownames(params)[which(params$nReducedDims == 50)]

ceSub <- makeConsensus(ce, whichClusters = clusterNames15, proportion = 0.7,
                       clusterLabel = "consensusPCA15")
ceSub <- makeConsensus(ce, whichClusters = clusterNames50, proportion = 0.7,
                       clusterLabel = "consensusPCA50")
```

Analogously, many visualization functions have a `whichClusters` argument that can be used to visually inspect the similarities and differences between subsets of clusterings.

```{r sub_visualize}
plotClusters(ceSub, whichClusters = c(clusterNames15, "consensusPCA15"))
plotClusters(ceSub, whichClusters = c(clusterNames50, "consensusPCA50"))
```
...
*makeDendrogram* | ignoreUnassignedVar=TRUE | dendroReduce      | reduceMethod | 
