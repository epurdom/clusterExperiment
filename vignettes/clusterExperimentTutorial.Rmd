...
system.time(rsecFluidigm<-RSEC(se, isCount = TRUE, 
	dimReduce="PCA", nPCADims=10,combineMinSize=3,
...
* set `dimReduce=c("PCA", "var")` meaning run the clustering algorithm trying two different methods for dimensionality reduction: the top principal componetns and filtering to the top most variable genes
* For PCA reduction, choose 5,15, and 50 principal components for the reduced data set (set `nPCADims=c(5,15,50)`)
* For most variable genes reduction, we choose 100, 500, and 1000 most variable genes (set `nFilter=c(100,500,1000)`)
...
ord<-order(test[,"k"],test[,"nFilter"],test[,"nReduceDims"])
...
ord<-order(cmParams[,"dimReduce"],cmParams[,"nReduceDims"])
...
ceDist<-clusterMany(ceDist, k=7:9, alpha=c(0.35,0.4,0.45), 
	 ...
	 findBestK=FALSE,removeSil=c(FALSE),dist=c("corDist","spearDist"),
          dimReduce=c("mad"),nFilter=1000,run=TRUE)
...
wh<-getClusterManyParams(ce)$clusteringIndex[getClusterManyParams(ce)$dimReduce=="var"]
