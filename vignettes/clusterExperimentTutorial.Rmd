...
Recall that the most recent run is of `combineMany` on only the ones with most variable genes, so there are only 3 clusters to merge (rather than the 5 found in quick start). The easiest way to do this is just redo `combineMany` and then call `makeDendrogram` on that output:
...
ce<-makeDendrogram(ce,dimReduce="mostVar",ndims=500)
ce<-mergeClusters(ce,cutoff=0.04,mergeMethod="adjP")
ce
```
...
Here we will go back to our previous mergeClusters that we found with `cutoff=0.05` and mark it as our final clustering. First we need to find which cluster it is. We will show all of the clusters that have clusterType "mergeClusters" in their name, regardless of the iteration:

```{r findMergeClusters}
data.frame( grep("mergeClusters",clusterType(ce)),clusterType(ce)[grep("mergeClusters",clusterType(ce))])
```

Now we mark the mergeClusters.1" version as the final one by changing it's clusterType manually to "final". 

```{r markFinal}
whFinal<-which(clusterType(ce)=="mergeClusters.1")
clusterType(ce)[whFinal]<-"final"
par(mar=plotCMar)
plotClusters(ce,whichClusters="workflow")
```

Note that because it is labeled as `final` it shows up automatically in "workflow" clusters. If we are settled on this one, then we probably want to also set it as the primaryCluster as well as change it's clusterLabel to be more meaningful for plots, etc. We also will make a dendrogram to go with it for later use (recall any dendrogram we had will be lost when we change the primaryCluster):
...
This didn't get rid of our undesired `mergeClusters` result that is most recent. It still shows up as the mergeClusters result. This might be undesired. We could remove it with `removeClusters` or manually change the clusterType to `mergeClusters.2` so that it doesn't show up as current. We can also change it's label, without changing its type, so as to be more informative.
