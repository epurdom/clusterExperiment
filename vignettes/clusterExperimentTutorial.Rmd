...
```{r plotClusterLegend_before, fig.width=3,fig.height=3}
...
```{r plotClusterLegend,fig.width=3,fig.height=3}
...
par(mfrow=c(2,2))
plotClusters(ce_temp, sampleData=c("Biological_Condition","Published2"),
             whichClusters="workflow", existingColors="firstOnly", clusterLabels=FALSE,
             main="firstOnly",axisLine=-1)

plotClusters(ce_temp, sampleData=c("Biological_Condition","Published2"),
             existingColors="ignore", whichClusters="workflow", clusterLabels=FALSE,
             main="default",
             axisLine=-1)
plotClusters(ce_temp, sampleData=c("Biological_Condition","Published2"),
           existingColors="all", whichClusters="workflow", clusterLabels=FALSE,
           main="useExisting",
           axisLine=-1)
```

We see that while the order of the samples has changed (because I have different set of clusters to align) the *colors* assigned to each cluster have stayed the same, so I can more easily compare the plots.

Note that the use of `existingColors="firstOnly"` and `existingColors="all"` will not give the same color assignments, *even if the colors have been previously aligned to be the same* unless its the exact same set of clusterings in the same order. This is because with each set of ordered clusterings, the realignment between clusters changes and so the assignment of colors changes. 

```{r plotClusterEx1_compareForceColors,fig.width=18,fig.height=6}
par(mfrow=c(2,2))
plotClusters(ce_temp, sampleData=c("Biological_Condition","Published2"), 
             existingColors="all", whichClusters="clusterMany", clusterLabels=FALSE,
             main="clusterMany Clusters, use existing colors",
