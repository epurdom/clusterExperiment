...
However, it doesn't take very long (roughly 1-2 minutes) so we recommend users try it themselves. The following code is the example of a common run of RSEC corresponding to the above object (the vignette does not run this code but the user can): 
...
However, to be sure that we exactly replicate the data object `rsecFluidigm` that is provided in the package, you would need to set a large number of parameters to make sure that the vignette is back-compatible (in different versions of the package, some defaults have changed). The code that created `rsecFluidigm` found with this version of the vignette can be found in the help file (`?rsecFluidigm`)
...
rsecFluidigm<-RSEC(rsecFluidigm,
 ...
 mergeMethod="adjP",
 mergeCutoff=0.01,
 consensusArgs=list(whenUnassign="after",   
