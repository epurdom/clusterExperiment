...
Notice that `nodeMergeInfo` gives the proportion estimated to be differentially expressed at each node, as well as whether that node was merged together in the last `mergeClusters` call (the `Merged` column). The column `mergeClusterId` tells us which nodes in the tree are now equivalent to a cluster; this is different than the `Merged` column, since some nodes can be merged but if their parent nodes were also merged, then that node will not be equivalent to a cluster in the "mergeClusters" clustering. 
