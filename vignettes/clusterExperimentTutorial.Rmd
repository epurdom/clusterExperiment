...
Here we create a heatmap that shows the clusters from the workflow. Note that we do that in a smaller object from which we removed most of the clusters. This is because `ce` now has 39 different clusterings, and these are too many to visualize on top of the heatmap.

```{r plotHeatmap_Ex1.1}
ce_sub <- removeClusters(ce, whichRemove = 4:39)
plotHeatmap(ce_sub,whichClusters="workflow", annLegend=FALSE)
```
...
plotHeatmap(ce_sub,clusterSamplesData="primaryCluster",whichClusters="all",main="Heatmap with clusterMany",annLegend=FALSE)
```
