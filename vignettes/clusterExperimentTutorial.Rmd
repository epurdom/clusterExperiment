...
plotDendrogram(ce)
```
Recall that the most recent run is of `combineMany` on only the ones with most variable genes, so there are only 3 clusters to merge (rather than the 5 found in quick start). Notice that the plot of the dendrogram shows the hierarchy of the clusters (and color codes them according to the colors stored in colorLegend slot).
