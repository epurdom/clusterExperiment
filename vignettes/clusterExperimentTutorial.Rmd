...
par(mar=plotCMar)
plotClusters(ce,main="Clusters from clusterMany", whichClusters=cmParams[ord,"clusteringIndex"], sampleData=c("Biological_Condition","Published2"), axisLine=-1)
```
...
clusterLabels(ce)<-gsub("Features","",clOrig)
plotClusters(ce,main="Clusters from clusterMany", whichClusters=cmParams[ord,"clusteringIndex"], sampleData=c("Biological_Condition","Published2"), axisLine=-1)
```

However, note that we changed the labels internally in `ce`, and therefore will not be able to run `getClusterManyParams` which relies on parsing the expected clusterLabels from `clusterMany`. 

```{r paramsError,error=TRUE}
head(getClusterManyParams(ce))
```
At this point, the best solution is to save the original cluster labels (as we did above) and reset the clusterLabels after you change them. 

```{r plotCluster_resetLabels}
clusterLabels(ce)<-clOrig
```
