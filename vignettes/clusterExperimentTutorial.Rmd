...
par(mar=plotCMar)
plotClusters(ce,whichClusters=wh,axisLine=-1)
```
...
Here we will go back to our previous mergeClusters that we found with `cutoff=0.05` and mark it as our final clustering. First we need to find which cluster it is. We see from our above call to the workflow functions above, that it is `mergeClusters.4` (we also see that if we had labelled these with clusterLabels, it would be much easier to track down!).
Now we mark the "mergeClusters.4" version as the final one by changing it's clusterTypes manually to "final". 
