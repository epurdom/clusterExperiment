...
In what follows, we'll go back to the original (default) RSEC settings by reloading the RSEC object (the original clusters are saved in the `rsecFluidigm` object, but there is useful information about the merging that is overwritten by our latest call so we will just rerun it to recreate the clustering). 
