...
## Creating a Hierarchy of Clusters  {#makeDendrogram}
...
The resulting dendrograms (one for just the cluster hierarchy and one that expands the cluster hierarchy to include the samples) are saved in the object. They are saved as a `phylo4d` class (from the package `phylobase`, which uses the basic format of the S3 class `phylo` in the `ape` package, but is a S4 class with some useful helpers). 
...
This shows the internal node ids, as well as which ones match to which dendrogram clusters (and if applicable which merged clusters). Generally, these will not need to be directly manipulated by the user. But if desired, the user can manipulate these objects using the functions in `phylobase`
...
To see more about the required format of these dendrograms, see `?clusterDendrogram`. 

The main reason to really work with these dendrograms directly is  to link it back with the merging results (see below)[#mergeClusters] or (feature extraction results)[#Dendrocontrasts]. 

In particular, one feature of the cluster dendrogram can be set by the user and be useful later on,  is the labels for the nodes. Just like the clusters, the nodes have permanent non-changing names (stored in the `NodeId` column). However, the internal node labels *of the cluster dendrogram* can be given user-defined names by accessed and chanced by calling the function `nodeLabels` *directly on the `ClusterExperiment` object*. We'll demonstrate this by giving the nodes new names that are the letters A-Z. The main trick, is that it is required that the new replacement vector have names that match the internal node id (the `NodeId` column). 
...
<!-- Add here info about changing the node labels -->
