...
Because this procedure is slightly computationally intensive, depending on the user's machine, we have set this code to not run so that the vignette will compile quickly. Changing `eval=TRUE` will run the code, which we recommend users try. On a 32-core compute machine with 62GiB of memory and a Quad-Core AMD Opteron(tm) Processor, running the following command took 1 minute (version `1.3.3-9001` of `clusterExperiment`). 
...
```
RSEC clusters the data many times using different parameters. Each clustering is the result of the combination of subsampling of the data and sequential clustering of the data. The different clusterings represent the different parameters that were adjusted. 
...
## Components of the clustering workflow
We explain superficially here what the standard workflow is that is used by RSEC and the  `clusterExperiment` package. 
...
These clustering steps are done in one function call by `RSEC`, described above, which is the preferred usage. However, to understand the parameters available in `RSEC` it is useful to go through the steps first. We will go through these steps, but instead of using the parameters of `RSEC` that involve subsampling and are computationally intensive, we will simplify the procedure so it just uses basic PAM with no subsampling or sequential clustering. This is simply for the purpose of understanding the object that is created, how to manipulate it, and the intermediate steps that `RSEC` follows. Later sections will go through these steps in more detail. 
