...
In particular, one feature of the cluster dendrogram can be set by the user and be useful later on,  is the labels for the nodes. Just like the clusters, the nodes have permanent non-changing names (stored in the `NodeId` column). However, the internal node labels *of the cluster dendrogram* can be set by the user by calling the function `nodeLabels<-` *directly on the `ClusterExperiment` object*. We'll demonstrate this by giving them the letters A-Z. The main trick, is that it is required that the new replacement vector have names that match the internal node id (the `NodeId` column). 
