...
This removed `r length(whLow)` genes out of `r NROW(fluidigm)` (`r length(whZero)` of which had zero in all samples). We now have `r NROW(se)` genes (or features) remaining.
...
```{r getBestFeatures}
pairsAll<-getBestFeatures(ce,type="Pairs",p.value=0.05,number=nrow(ce),voomCorrection=TRUE)
head(pairsAll)
```
...
Instead `combineMany` has a parameter `proportion` that governs in what proportion of clusterings the samples should be together. Internally, `comineMany` makes a coClustering matrix $D$, similar to that of subsampl


Some range of parameters make better choices to `combineMany` across. 
