...
* Clustering an input $D$ matrix `clusterD`, which can be either the co-occurance matrix from `subsampleClustering` or otherwise is `dist(x)`. The arguments passed to `clusterD` are via the argument `clusterDArgs` in `clusterAll`. The most important arguments are:
  * `D` the $n\times n$ matrix of dissimilarities
  * `clusterFunction` the type of clustering to do on the dissimilarity matrix.
  * Both of the above are handled by the arguments to `clusterAll`. Depending on the choice of `clusterFunction` some other arguments may be necessary to pass via `clusterDArgs` (e.g. if "pam", must also pass an argument `k` or `findBestK=TRUE`)
...
simpleCluster<-clusterAll(simData, subsample=FALSE, sequential=FALSE, clusterFunction="pam",clusterDArgs=list('k'=4))
library(cluster)
...
```{r basicSubsampling}
#library(clusterPackage)
subsampleCluster<-clusterAll(simData, subsample=TRUE, sequential=FALSE, clusterFunction="pam",clusterDArgs=list('k'=4))
...
```{r basicSubsampling2}
subsampleCluster2<-clusterAll(simData, subsample=TRUE, sequential=FALSE, clusterFunction="pam",clusterDArgs=list('k'=4),subsampleArgs=list("k"=2))
