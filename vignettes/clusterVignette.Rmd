...
There are two main user-functions, `clusterAll` and `compareChoices`. `clusterAll` is the wrapper function that calls the underlying functions and `compareChoices` runs `clusterAll` across combinations of parameter choices. Another useful function is `subsampleClustering` which does subsampling of the data and returns a $nxn$ matrix $D$ of the proportion of co-occurances of samples in the same cluster.


There are three main choices that the user needs to make for `clusterAll`:
...
The `clusterAll` calls underlying functions for each of these tasks, each of which can take many arguments. For simplicity in syntax, the user passes arguments to each of these underlying functions as lists of arguments. 
...
For this reason, the next sections will go through the functions that form the components of `clusterAll` so as to better explain the possible options that can be passed. 
...
* (Optional, if `subsample=TRUE`) Subsample data and cluster it repeatedly via the function `subsampleClustering`. The arguments passed to `subsampleClustering` are via `subsampleArgs` in `clusterAll`. The most important arguments that a user may need to pass to `subsampleCluster` are:

  * `x` the $n\times p$ data matrix (which is handled by the argument `x` to `clusterAll` so the user does not need to set it)
  ...
  * `clusterFunction` the function to use to cluster the subsampled data. The default is `pam` from the `cluster` package but can also be user-defined. The argument `clusterFunction` of `clusterAll` is *not* passed to this argument -- `clusterAll` by default uses the `pam` function -- so the user must pass this *via `subsampleArgs`* if they want a different function.

* Clustering an input $D$ matrix `clusterD`, which can be either the co-occurance matrix from `subsampleClustering` or otherwise is calculated as `dist(x)`. The arguments passed to `clusterD` are via the argument `clusterDArgs` in `clusterAll`. The most important arguments are:
  * `D` the $n\times n$ matrix of dissimilarities
  * `clusterFunction` the type of clustering to do on the dissimilarity matrix.
  * Note that both of the above options are direct arguments of `clusterAll` (i.e. do not need to be passed with a list of arguments for). Therefore `clusterFunction` argument of `clusterAll` is passed to `clusterD` (and not to `subsampleClustering`)
...
data(simData)
simpleCluster<-clusterAll(simData, subsample=FALSE, 
...
subsampleCluster<-clusterAll(simData, subsample=TRUE, 
                             sequential=FALSE, clusterFunction="pam",clusterDArgs=list('k'=4))
...
```{r basicSubsampling2}
subsampleCluster2<-clusterAll(simData, subsample=TRUE, sequential=FALSE,
...
The following image shows how these clustering functions are put together by `clusterAll` to form the basic clustering sequence, where the functions are indicated by ovals in the flowchart. 
...
fromtoChar<-rbind(c("1:clusterAll","2:sequential"),c("2:sequential","3:Basic Clustering\n Routine"),c("2:sequential","4:seqCluster"),
...
# Compare many parameters (`compareChoices`)

`compareChoices` is a function for running many different combinations of parameters or different datasets. It's main goal is to make it easy to running many different parameter choices and reduce the coding overhead for doing so. `compareChoices` calls `clusterAll` using `lapply` (or `mclapply` if parallelizing the computation). Example uses of `compareChoices` would be to run the same clustering over data sets representing different number of dimensions in a PCA dimensionality reduction, or to run many different choices of `alpha` in clusterD clustering algorithm, or to compare clustering based on different algorithms -- or all of the above!

`compareChoices` basically takes all combinations of all the user defined parameters and runs `clusterAll` across these combinations (basically `expand.grid`). Along the way, the function removes some obviously irrelevant combinations (e.g. setting combinations that vary only in `alpha` and `findBestK` are irrelevant since those are options for entirely different clustering algorithms and various combinations of them will never be used). 

The user can choose `run=FALSE` in the call to `compareChoices` to skip actual evaluation of the clusterings, and instead return just a matrix providing the set of parameter combinations that are implied by the user's call to `compareChoices`. This allows the user, for example, to see how many different combinations their call implies! Or how many of them involve `sequential=TRUE` or `subsample=TRUE`. It also allows the user to see what the colnames of the output they *would* get if they ran `compareChoices`, which can aid further coding while the computationally expensive command is being run. Setting `run=FALSE` also allows the user to prune some calls out (see below). Setting `run=FALSE` is recommended first step before launching computationally expensive clustering runs over many parameters. 

## User-defined choices of parameters

`compareChoices` does allow for some mild tinkering with the parameters via the argument `paramMatrix` where the user can provide a matrix of parameter choices. There are very few checks of this input and the function is not robust to invalid choices! The intended use-case for the `paramMatrix` argument is that a user does not want to run *all* combinations of their choices -- only a subset of them -- yet still wants to make use of the `compareChoices` function to avoid additional coding. The user can run `compareChoices` with `run=FALSE` to get the internally-created `paramMatrix`. Then the user can delete some of the combinations simply by deleting some rows of the full `paramMatrix` and feed the pruned matrix back into `compareChoices`.
