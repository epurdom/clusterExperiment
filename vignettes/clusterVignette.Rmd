...
* Clustering is done via `clusterD` and arguments to `clusterD` are passed as a list via the argument `DclusterArgs`. See Section XXX below on `clusterD` function for more details about possible arguments.
...
First we will discuss if `sequential=FALSE`, i.e. the user does not choose to use the sequential removal of clusters.  Then the algorithm performs what we call the "basic clustering sequence"; this basic sequence is called iteratively by the sequential method.
...
* Clustering an input $D$ matrix `clusterD`, which can be either the co-occurance matrix from `subsampleClustering` or otherwise is `dist(x)`. The arguments passed to `clusterD` are via the argument `DclusterArgs` in `clusterAll`. The most important arguments are:
  * `D` the $n\times n$ matrix of dissimilarities
  * `clusterFunction` the type of clustering to do on the dissimilarity matrix.
  * Both of the above are handled by the arguments to `clusterAll`. Depending on the choice of `clusterFunction` some other arguments may be necessary to pass via `DclusterArgs` (e.g. if "pam", must also pass an argument `k` or `findBestK=TRUE`)
...
simpleCluster<-clusterAll(simData, subsample=FALSE, sequential=FALSE, clusterFunction="pam",DclusterArgs=list('k'=4))
library(cluster)
...
```{r basicSubsampling}
#library(clusterPackage)
subsampleCluster<-clusterAll(simData, subsample=TRUE, sequential=FALSE, clusterFunction="pam",DclusterArgs=list('k'=4))
table(pamCluster$clustering,subsampleCluster$clustering)
```

Notice that we got a warning. This is because we *could* have chosen to cluster with a different number of clusters on the subsampled data than we clustered the co-occurance data. For example,

```{r basicSubsampling2}
subsampleCluster2<-clusterAll(simData, subsample=TRUE, sequential=FALSE, clusterFunction="pam",DclusterArgs=list('k'=4),subsampleArgs=list("k"=2))
...
row.names(fromto)<-c("TRUE","FALSE",NA,'"01"','"K"',NA,NA,'"01"','"K"')
nr     <- nrow(fromto)
############
###Draw Arrows:
############
...
```{r flowspreadSeq, echo = F, fig.width=10,fig.height=15,out.height=300*1.5,out.width=300}
#results = 'asis'}
...
curvedarrow(from = elpos["15:Restart with\nk.start=k.start-1\n(but no less than k.min)", ], to = elpos["5:Set k.start=k0\nn=No.samples", ]+c(.05,0),  lty = 1, lwd=10, lcol = 2,arr.pos=0.9,curve=0.5)
selfarrow(pos=elpos["6:Iterate Basic\nClustering Routine\nk=k.start,kstart+1,...", ]+c(.1,0), path="R", lty = 1, lwd=2, lcol = 2,arr.pos=.4,curve=c(0.1,0.05))
###########
## Draw boxes
###########
nicenames<-sapply(strsplit(row.names(elpos),":"),.subset2,2)
