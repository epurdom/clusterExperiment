...
* we will unassign samples from clusters if they have negative silhouette scores (`removeSil=TRUE`)
...
We can visualize multiple clusterings using the `plotClusters` command.
...
We can also add (categorical) information from the `colData` of our fluidigm object, which is also retained in our ClusterExperiment object. This can be helpful to see if the clusterings correspond to other features of the samples, such as sample batches. 
