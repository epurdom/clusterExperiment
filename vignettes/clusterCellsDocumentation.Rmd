...
There are two main user-functions, `clusterAll` and `clusterMany`. `clusterAll` is the wrapper function that calls the underlying functions and `clusterMany` runs `clusterAll` across combinations of parameter choices. Another useful function is `subsampleClustering` which does subsampling of the data and returns a $nxn$ matrix $D$ of the proportion of co-occurances of samples in the same cluster.
