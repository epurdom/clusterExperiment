...
library(diagram)
drawClusterAll <- function(highlight=NULL,
Layer2Options=NULL,plotOptions=TRUE,Layer2OnlyIfUsed=TRUE){
	...
	Layer2Names<-c("clusterDArgs\n-->clusterD( )","subsampleArgs\n-->subsampleClustering( )","seqArgs\n-->seqCluster( )")
	...
	values<-"ellipse"
	options<-"rect"
	Layer1Pos<-list("clusterFunction"=makeDf(0.25, 0.77,0.06,0.4,options),"subsample"=makeDf(0.50,0.77,0.06,0.4,options), "sequential"=makeDf(0.75, 0.77,0.06,0.4,options))
	Layer1OptionsPos<-list(
		"clusterFunction"=list("tight"=makeDf( 0.081, 0.59,0.03,0.4,values), "hierarchical"=makeDf(0.162, 0.59,0.045,0.4,values), "pam"=makeDf(0.243, 0.59,0.03,0.4,values), "kmeans"=makeDf(0.324, 0.59,0.03,0.4,values)), 
	...
	Layer2Pos<-list("clusterDArgs"=makeDf(0.2,0.44,0.08,0.4,options), 
		"subsampleArgs"=makeDf(0.5,.44,0.08,0.4,options), 
		"seqArgs"=makeDf(0.825, 0.44,0.08,0.4,options))
	...
	if(length(Layer2Options)>0){
		Layer2OptionsPos<-lapply(names(Layer2Options),function(nam){
			...
			ys<-0.44-cumsum(rep(0.05,length(opts)))
			 data.frame(x=Layer2Pos[[nam]][,"x"], y=ys, box.size=0.03,box.prop=0.4,box.type=options,boxCol="red")
		})
...
#	print(M)
	...
	par(mar=c(1, 1, 2, 1))
	diagram::plotmat(M, pos = data.matrix(posMatrix[,c("x","y")]), name = names, absent="0",lwd = 1, box.lwd = 2, box.cex = 0.5, box.size = posMatrix[,"box.size"], box.type = posMatrix[,"box.type"], box.prop = posMatrix[,"box.prop"], curve = 0, main="", arr.type="triangle", arr.width=0, arr.length=0, box.lcol=posMatrix[,"boxCol"])
...
We can visualize these as a diagram: 

```{r, echo=FALSE, message=FALSE}
drawClusterAll(Options=1,Layers=c(1))
```
...
```{r, echo=FALSE}
drawClusterAll(Options=1,Layers=1,highlight = c("pam", "subsample.FALSE", "sequential.FALSE"))
```
