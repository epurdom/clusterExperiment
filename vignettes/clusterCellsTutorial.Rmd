...
Let us start with a walkthrough of `clusterAll`. As a wrapper function, `clusterAll` can receive a variety of arguments to perform different clustering algorithms. Here is the usage of `clusterAll`: 
```{r, eval=FALSE}
...
library(diagram)
drawClusterAll = function(arguments=NULL, subsample=NULL){
  names = c("clusterAll", "clusterFunction", "subsample", "sequential", "tight", "hierarchical", "pam", "kmeans", "no", "yes", "no", "yes", "clusterFunction", "pam", "kmeans")
  if(is.null(arguments)){colargs="black"}else{
    ...
    if (arguments[3] == "no"){colargs[11] = "red"}else{colargs[12] = "red"}
  }
  par(mar=c(1, 1, 2, 1))
  M = matrix(nrow=15, ncol=15, byrow=TRUE, data=0)
...
```{r, echo=FALSE}
drawClusterAll(arguments = c("pam", "no", "no"))
```
...
```{r, echo=FALSE}
drawClusterAll(arguments = c("tight", "yes", "no"), subsample = "kmeans")
```
```{r}
...
```{r, echo=FALSE}
drawClusterAll(arguments = c("hierarchical", "yes", "yes"), subsample = "kmeans")
```
```{r}
