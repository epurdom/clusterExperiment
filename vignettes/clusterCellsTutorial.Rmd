...
knitr::opts_chunk$set(fig.align="center", cache=TRUE, cache.path = "R_cache/", fig.path="R_figure/", fig.width=6,fig.height=6,error=TRUE,autodep=TRUE)
knitr::dep_auto()
library(diagram)
...
#	print(M)
	...
	par(mar=c(1, 1, 2, 1))
	diagram::plotmat(M, pos = data.matrix(posMatrix[,c("x","y")]), name = names, absent="0",lwd = 1, box.lwd = 2, box.cex = box.cex, box.size = posMatrix[,"box.size"], box.type = posMatrix[,"box.type"], box.prop = posMatrix[,"box.prop"], curve = 0, main="", arr.type="triangle", arr.width=0, arr.length=0, box.lcol=posMatrix[,"boxCol"])
...
We provide a visualization to compare many clusterings of the same data in the function `plotTracking`.  We also provide some functions that are wrappers for common visualization tasks in clustering gene expression data. We provide an heatmap function that is an interface to the `aheatmap` function in `NMF` package. Our wrapper simply does some simple pre-processing common to clustering. 
...
Let us start with a walkthrough of `clusterAll`.  `clusterAll` is the primary function for performing the primary clustering tasks. Each of these tasks has a underlying (user-accessible) function and `clusterAll` is a wrapper around these tasks:
...
Let us use the function `clusterAll` to perform simple PAM clustering on our expression data set with `k=5`. In this expression matrix, rows are samples and columns are features. 

To do standard pam clustering, we set `clusterFunction="pam"`. The remaining tasks (subsampling and sequential), we will not perform, so we set `subsample=FALSE` and `sequential=FALSE`, which we can visualize as:

```{r, echo=FALSE}
drawClusterAll(highlight = c("pam", "subsample.FALSE", "sequential.FALSE"))
```

However, notice that doing this gets me an error:
```{r}
simpleCluster = clusterAll(expressions, subsample=FALSE, sequential=FALSE, clusterFunction="pam")
```
This is because even to perform just simple PAM, the PAM algorithm needs additional parameters to be set, namely to choose 'k' the number of groups. 
...
**Note:** at the current time, if we are not going to do the subsample task (`subsample=FALSE`), then `clusterFunction` *must* be `pam` because the other clustering options for `clusterFunction` assume a 0-1 distance matrix based on subsampling. If there is no subsampling, `clusterAll` makes the distance matrix by default with `dist(x)` and therefore must be used with `pam`. 
...
Again, we need to pass arguments to control how this is done, now as a list to the argument `seqArgs`. A necessary argument `k0` must be passed to `seqArgs`. What this argument means depends on what other choices are being made, but if we also set `subsample=TRUE` (recommended), then `k0` is the value of K passed to the subsampling clustering method at the first iteration of sequential algorithm. 
...
`getBestGenes` will be called to determine the features most associated pairwise comparisons between the clusters. By default, `topTable` in `limma` only returns the top 10, which `getBestGenes` will mean 10 per each of the pairwise comparisons.
...
head(pairsAll)
```
The column `Cluster`  in the output signifies the pairwise difference (or contrast) for which the information on the row comes from. `IndexInOriginal` gives matches the gene to the index of the gene in the original dataset. The other columns are provided by `topTable` in `limma` (see documentation therein).
...
Note that by using contrasts, we are making use of all of the data, not just the samples in the particular cluster pairs being compared. This means the variance is estimated with all samples.

## Heatmaps of Marker Genes

One way to visualize samples with only the features found by `getBestGenes` is to use the `dualHeatmap` function. The `dualHeatmap` function minimally requires two arguments - a vector of cluster assignments and the data matrix. `dualHeatmap` uses the heatmap function `aheatmap` in the package `NMF`, but provides an interface that is useful for the standard plots desired for evaluating the clustering output. In particular, it is designed to draw the cluster assignments of the samples on the top, where different colors denote different cluster assignments; lack of assignment, denoted by "-1", are automatically colored white.  In the code example below, `clusterData` is set to the data matrix to define the color scale. Currently, `dualHeatmap` takes the log of the input data (i.e. argument `heatData`); this will be fixed to be an option in future versions, but currently to not do this requires exponentiating the data.

```{r}
dataMatrix = simData[, unique(pairsAll2$IndexInOriginal)]
cluster = cl$clustering
dualHeatmap(cluster, exp(dataMatrix), eps=0, clusterData=dataMatrix)
```

Additional covariates can also be plotted by giving a matrix of values to `annCol`; unlike `aheatmap`, `dualHeatmap` will assume that such values should be converted to factors unless told otherwise by the argument `whAnnCont`. 


Another feature of dualHeatmap is that a different dataset can be used for determining the clustering than for determining the color-scale image of the heatmap. This can be useful, for example, for clustering on the normalized data which is not on the count-scale, but showing the original count-scale in the color-scale which is more interpretable. 
...
More usefull, however, is to use this in conjunction with dualHeatmap, in which case we want the individual samples in the dendrogram, not just the clusters. Notice that `full=TRUE` is necessary to get such a dendrogram. Then we give the data as `heatData` to `dualHeatmap` indicate it should be used for creating the color-image of the data, but provide our dendrogram to `clusterData` to indicate it will provide the clustering of the data. 

```{r}
clDendroFull<-clusterHclust(dataMatrix,cl$clustering,full=TRUE,unassigned="outgroup")
dualHeatmap(cl$clustering,heatData=exp(dataMatrix),eps=0,clusterData=clDendroFull)
```
...
I could also choose to remove them, but I then need to make sure that my input data matrix to `dualHeatmap` doesn't contain these samples.

```{r, warning=FALSE}
clDendroFull<-clusterHclust(dataMatrix,cl$clustering,full=TRUE,unassigned="remove")
whRm<-which(cl$clustering>0)
dualHeatmap(cl$clustering[whRm],heatData=exp(dataMatrix)[whRm,],eps=0,clusterData=clDendroFull)
```
...
head(dendroGenes)
```
The output is similar to before, only now there are two identifies for the contrast: one is `ContrastName` which gives the Node being compared, and the other is `Contrast` which is the actual mathematical statement of the mean comparison between clusters that is being performed.

We can use this basic idea to merge clusters based on what percentage of genes are found differentially expressed. The `mergeClusters` function performs such contrasts for each node, and then calculates the proportion of significant hypotheses. There are several methods implemented, but the simplest is just the proportion found by doing BH adjusted p-values and determine the proportion that are DE. 

```{r}
mergeOut<-mergeClusters(dat=simData, cl=cl$clustering, mergeMethod = c("adjP"), cutoff = 0.1, plotType = "mergeMethod")
table(mergeOut$cl,cl$clustering)
...
# Running and Comparing Many Cluster Choices (`compareChoices`)
...
diagram::plotmat(M, pos=c(1, 4, 1, 2), name=names, lwd=1, box.lwd=2, box.cex=0.5, box.size=0.08, box.type=c(rep("square", 5), "diamond", rep("square", 2)), box.prop=0.2, curve=0, main="compareChoices Workflow", arr.type="triangle", arr.width=0, arr.length=0, box.lcol=colargs)
par(mar=c(5.1, 4.1, 4.1, 2.1))
}
```
`compareChoices` is a function for running many different combinations of parameters or different datasets. All combinations given by the user are run through `clusterAll` (though the function does some (minimal) checking to remove nonsensical combinations). 
...
After `compareChoices` is run, the function `plotTracking` can be used to visualize the many different cluster assignments that resulted, and `findSharedClusters` can be used to determine a consensus clustering across the parameter combinations. And `mergeClusters` described above can be used to help the user determine whether some clusters in the consensus clustering should be merged.
...
## Compare Choices
In this example, the combination of parameters we are testing are the range of k values for clustering and whether `removeSil` is TRUE or FALSE, and whether `pam` or `kmeans` is used.

Before running the clustering on these combinations, it is a good idea to first inspect what parameter combinations will be run by calling `compareChoices` and setting `run`=FALSE. The resulting output is a matrix where each column corresponds to a parameter setting and each row is a combination of parameters that will be sent to `clusterAll`.

```{r}
parameters = compareChoices(expressions, ks=c(3:7), clusterMethod=c("pam", "kmeans"), run=FALSE)
head(parameters)
dim(parameters)
```

**Note:** The output can be fed back into `compareChoices`. For example if some combinations are not of interest, those rows could be removed and then the resulting matrix given as an argument

Let us now run `compareChoices` on the original data set we simulated.

```{r}
result = compareChoices(expressions, ks=c(3:7), clusterMethod=c("pam"), subsample=FALSE, removeSil = c(TRUE, FALSE))
head(result$clMat)
```
...
To visualize the several clusters created by `compareChoices`, we run `plotTracking` to visualize the assignments.  `plotTracking` minimally takes as its argument a matrix of cluster assignments where each row represents a sample and each column a clustering. The result of `plotTracking` is a color representation of the clusters, where each column of the plot is a sample, and the rows are the different clusterings (note this is the inverse of the input clustering matrix so as to conserve space). Within each row/clustering, the samples are color coded as to their cluster assignment. Furthermore, `plotTracking` tries to order the samples so that the clusters are assigned colors that make clusters composed of similar samples get the same cluster color. 
...
table(r$clustering)
plotTracking(cbind(FINAL=r$clustering,result$clMat))
```
...
```{r}
dualHeatmap(clusterVector=r$clustering,heatData=exp(r$percentageShared),eps=0)
```
