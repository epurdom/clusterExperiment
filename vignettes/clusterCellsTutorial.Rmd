...
```{r}
dualHeatmap(cluster, exp(dataMatrix), eps=0, clusterData=dataMatrix, annCol = data.frame(clusterCells=cluster, TrueClusters=trueCluster))
```
...
More useful, however, is to use this in conjunction with `dualHeatmap`, in which case we want the individual samples in the dendrogram, not just the clusters. Notice that `full=TRUE` is necessary to get such a dendrogram. Then we give the data as `heatData` to `dualHeatmap`, indicating that it should be used for creating the color-image of the data, but provide our dendrogram to `clusterData` to indicate it will provide the clustering of the data. 

```{r dualHeatmapHclust}
clDendroFull<-clusterHclust(dataMatrix,cl$clustering,full=TRUE,unassigned="outgroup")
dualHeatmap(cl$clustering,heatData=exp(dataMatrix),eps=0,clusterData=clDendroFull,annCol = data.frame(clusterCells=cluster, TrueClusters=trueCluster))
```
...
One could also choose to remove them, making sure that the input data matrix to `dualHeatmap` doesn't contain these samples.

```{r, warning=FALSE}
clDendroFull<-clusterHclust(dataMatrix,cl$clustering,full=TRUE,unassigned="remove")
whRm<-which(cl$clustering>0)
dualHeatmap(cl$clustering[whRm],heatData=exp(dataMatrix)[whRm,],eps=0,clusterData=clDendroFull,annCol = data.frame(clusterCells=cluster[whRm], TrueClusters=trueCluster[whRm]))
```
...
Let us now run `compareChoices` on the original simulated dataset.
