...
setMethod(
	...
	if(!is.numeric(whichClusters)){
	  test<-try( match.arg(whichClusters[1], c("workflow")), silent=TRUE)
	      ...
	      if(!is.null(ppIndex) && sum(ppIndex[,"iteration"]==0)>0){
	        wh<-unlist(lapply(.workflowValues,function(tt){
	          ppIndex[ppIndex[,"iteration"]==0 & ppIndex[,"type"]==tt,"index"]
			  	...
			  	wh<-ClusterExperiment:::.createMismatch("There are no workflow clusterings")
		  	}
	  	}
		}
		else wh<-callNextMethod()
	}
	else wh<-callNextMethod
	return(wh)
