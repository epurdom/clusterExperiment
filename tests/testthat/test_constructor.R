...
test_that("accessing transformed data works as promised",{
  ...
  # need add checks for other objects as input...
  expect_silent(dr<-makeDimReduce(simData,dimReduce="PCA",maxDims=3))
  expect_silent(dr<-makeDimReduce(seSimData,dimReduce="PCA",maxDims=3))
  expect_silent(dr<-makeDimReduce(sceSimData,dimReduce="PCA",maxDims=3))
