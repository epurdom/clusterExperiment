...
test_that("`getClusterIndex` works", {
  ...
  expect_silent(ccM <- addClusterings(ccM, vec,clusterTypes="new"))
	...
	expect_equal(length(indCM),sum(clusterTypes(ccM)=="User"))
	expect_equal(getClusterIndex(ccM,wh=c("mySpecialLabel","new")),match(c("mySpecialLabel","new"),clusterTypes(ccM)))
	expect_equal(getClusterIndex(ccM,wh=c("mySpecialLabel","User")),c(match(c("mySpecialLabel"),clusterTypes(ccM)),indCM))
	expect_equal(getClusterIndex(ccM,wh=c("mySpecialLabel","garbage"),noMatch="silentlyRemove"),
...
test_that("adding to colData works",{
	expect_silent(ccNamed<-renameClusters(ccSE,whichCluster="primary",value=letters[1:5]))
	expect_silent(df1<-colDataClusters(ccNamed,"Cluster1",useNames=FALSE,makeFactor=FALSE))
	expect_true(is.numeric(df1$Cluster1))
	expect_silent(df1<-colDataClusters(ccNamed,"Cluster1",useNames=FALSE,makeFactor=TRUE))
	expect_true(is.factor(df1$Cluster1))
	expect_silent(df1<-colDataClusters(ccNamed,"Cluster1",useNames=TRUE,makeFactor=FALSE))
	expect_true(is.character(df1$Cluster1))
	expect_silent(df1<-colDataClusters(ccNamed,"Cluster1",useNames=TRUE,makeFactor=TRUE))
	expect_true(is.factor(df1$Cluster1))

	expect_silent(x1<-addToColData(ccNamed,"Cluster1"))
	expect_equal(colData(x1),colDataClusters(ccNamed,"Cluster1"))
})
