...
test_that("Fast PCA gives the same results as PCA", {
  ...
  pca_res <- prcomp(mat)
  pca_res2 <- clusterExperiment:::.pca(mat, k=NCOL(mat))
  expect_equivalent(pca_res$x, pca_res2)

  pca_res <- prcomp(mat, center=FALSE)
  pca_res2 <- clusterExperiment:::.pca(mat, k=NCOL(mat), center=FALSE)
  expect_equivalent(pca_res$x, pca_res2)
  ...
  pca_res2 <- clusterExperiment:::.pca(mat, k=NCOL(mat), scale=TRUE, center=FALSE)
  expect_equivalent(pca_res$x, pca_res2)
