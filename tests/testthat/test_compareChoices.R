...
test_that("`compareChoices` works with matrix, list of data, ClusterCells objects, and
          SummarizedExperiments", {
            clustNothing <- compareChoices(simData, ks=c(3,4),clusterMethod="pam",
            ...
            clustNothing2 <- compareChoices(se, ks=c(3,4),clusterMethod="pam",
                                            subsample=FALSE, sequential=FALSE,
                                            isCount=FALSE)
            ...
            clustNothing3<- compareChoices(test, ks=c(3,4),clusterMethod="pam",
                                           subsample=FALSE, sequential=FALSE,
                                           isCount=FALSE)
            clustNothing4<- compareChoices(clustNothing3, ks=c(3:4),clusterMethod="pam",
                                           subsample=FALSE, sequential=FALSE,
                                           isCount=FALSE,eraseOld=TRUE) #hitting error. Need to go back (infinite recursion -- why?)
            expect_equal(clustNothing3,clustNothing4)
            clustNothing5<- compareChoices(clustNothing3, ks=c(5:6),clusterMethod="pam",
                                           subsample=FALSE, sequential=FALSE,
            ...
            #check dim reduce 
            cc <- compareChoices(simData, ks=c(3,4),nVarDim=c(15,20),nPCADim=c(3,4),dimReduce=c("none","PCA","mostVar"),clusterMethod="pam",
                                           subsample=FALSE, sequential=FALSE,
                                           isCount=FALSE)
            #check giving paramMatrix
            param <- compareChoices(simData, ks=c(3,4),nVarDim=c(15,20),nPCADim=c(3,4),dimReduce=c("none","PCA","mostVar"),clusterMethod="pam",
                                           subsample=FALSE, sequential=FALSE,run=FALSE,
                                           isCount=FALSE)
            cc2 <- compareChoices(simData, ks=c(3,4),nVarDim=c(15,20),nPCADim=c(3,4),dimReduce=c("none","PCA","mostVar"),clusterMethod="pam",
                                           subsample=FALSE, sequential=FALSE,
