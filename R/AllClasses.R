...
setValidity("ClusterExperiment", function(object) {
	...
	ch1<-.checkClusterMatrix(object)
	if(is.logical(ch1) && ch1) ch2<-.checkDendrogram(object) else return(ch1)
		if(is.logical(ch2) && ch2) ch3<-.checkCoClustering(object) else return(ch2)
			if(is.logical(ch3) && ch3) ch4<-.checkPrimaryIndex(object) else return(ch3)
	  		  if(is.logical(ch4) && ch4) ch5<-.checkClusterTypes(object) else return(ch4)
				  if(is.logical(ch5) && ch5) ch6<-.checkClusterLegend(object) else return(ch5)
					  if(is.logical(ch6) && ch6) ch7<-.checkOrderSamples(object) else return(ch6)
						  if(is.logical(ch7) && ch7) return(TRUE) else return(ch7)
})
