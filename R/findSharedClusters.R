...
#'to `findSharedClusters` and the \code{percentageShared} matrix stored in the
...
setMethod(
  ...
  if(proportion == 1) {
    ...
    if(is.character(clusterFunction)){
      ...
      if(typeAlg!="01") {
        stop("findSharedClusters is only implemented for '01' type clustering functions (see help of clusterD)")
      }
    }
...
setMethod(
  ...
  definition = function(x, whichClusters, proportion=1,
    ...
    outlist <- findSharedClusters(clusterMat, proportion=proportion,
    ...
    newObj <- clusterExperiment(x, outlist$clustering,
                                ...
                                clusterType="findSharedClusters")
    clusterLabels(newObj) <- "findSharedClusters"
...
setMethod(
  ...
  definition = function(x, whichClusters, proportion=1,
    ...
    findSharedClusters(x, wh, proportion=proportion,
                       clusterFunction=clusterFunction,
                       propUnassigned=propUnassigned,
                       minSize=minSize)
  }
