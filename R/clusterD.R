...
#' @param typeAlg character value of either '01' or 'K' determining whether the function given in clusterFunction should be called by clusterK or cluster01. Methods provided by the package can also be used by setting clusterFunction to a character vector in which case, clusterD will determine the appropriate input for 'typeAlg'.
#' @param min.size the minimum number of samples in a cluster
...
#' @param k single k value to be used if findBestK=FALSE
...
#' @param removeSil logical as to whether remove with silhouette < alpha
#' @param ... arguments given to clusterD to be passed to cluster01 or clusterK (depending on the value of typeAlg). Examples include 'k' for clusterK or 'alpha' for cluster01
#' 
...
.hierClusterDMat<-function(D,alpha,method=c("minimum","average"),...)
{
...
.tightClusterDMat <- function(D, alpha, min.size.core) 
{
