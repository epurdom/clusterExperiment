...
#' 'ContrastName', if given.. If  missing, default colors given by match to the cluster names of 
#' \code{whichCluster} (see above), or otherwise given a default assignment.
#' @param ... Arguments passed to \code{\link{plotHeatmap}}
#' @details If the column 'ContrastName' is given in \code{signifTable}, these names will be used to describe the contrast in the legend. 
#' @details Within each contrast, the genes are sorted by log fold-change 
#' if the column "logFC" is in the \code{signifTable} data.frame
