...
.hier01Cluster<-function(diss,alpha,evalClusterMethod=c("maximum","average"),whichHierDist=c("as.dist","dist"),checkArgs,cluster.only,...)
{
	...
	passedArgs<-.getPassedArgs(FUN=fastcluster::hclust,passedArgs=list(...) ,checkArgs=checkArgs)
	S<-round(1-diss,10)
	d<-switch(whichHierDist,"dist"=dist(S),"as.dist"=as.dist(diss))
	hDmat<-do.call(fastcluster::hclust,c(list(d=d),passedArgs))
...
.hierKCluster<-function(diss,k,checkArgs,cluster.only,...){
	passedArgs<-.getPassedArgs(FUN=fastcluster::hclust,passedArgs=list(...) ,checkArgs=checkArgs)
    hclustOut<-do.call(fastcluster::hclust,c(list(d=as.dist(diss)),passedArgs))
...
#'   the argument 'k'
#' Input is \code{"X"}; algorithm type is "K"}
#' \item{"hierarchical01"}{\code{\link[fastcluster]{hclust}} in
...
#' \item{"hierarchicalK"}{\code{\link[fastcluster]{hclust}} in \code{stats} package is used
