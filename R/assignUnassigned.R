...
#' @param whichClusters in which clustering to assign the unassigned samples
...
#' @inheritParams addClusters
#' @export
setMethod(
  f = "assignUnassigned",
  signature = signature("ClusterExperiment"),
  definition = function(x,whichClusters="primary",clusterLabel,makePrimary=TRUE,whichAssay=1,reduceMethod="none",nDims=defaultNDims(x,reduceMethod)){
    ...
    cl<-clusterMatrix(x)[,whCl]
		...
		if(length(whichUnassigned)>0){
			    ...
			    rownames(medoids) <- levels(clFactor)
					...
					###############
					cl[cl<0]<-classif
					x<-addClusters(x,cl, clusterLabel= clusterLabel,makePrimary=makePrimary)
					return(x)
			}
			else{
