...
#' @param pairMat pairs of clusters for which to do pair-wise contrasts (must match to elements of cl). If NULL, will do all pairwise of the cl (excluding "-1")
#' @param returnType Whether to return the index of genes, or the full table given by topTable or topTableF.
#' @param requireF whether a significant omnibus F-statistic should be required before testing for significant contrasts (only for option type="Dendro" or "Pairs"). In which case the user must supply an option 'p.value' to be sent to 
...
#'
#' @details  Note that the default option for \code{\link{topTable}} is to not filter based on adjusted p-values (\code{p.value=1}) and return only the top 10 most significant (\code{number=10}) -- these are options the user can change (these arguments are passed via the \code{...} in getBestGenes. In particular, it only makes sense to set \code{requireF=TRUE} if \code{p.value} is meaningful (e.g. 0.1 or 0.05); the default value of \code{p.value=1} will not result in any effect on the adjusted p-value. 
...
#	require(limma)
	...
	tops<-switch(type,"F"=.getBestFGenes(fit,...),
		"Dendro"=.getBestDendroGenes(cl,fit,dendro,contrastAdj=contrastAdj,...),
		"Pairs"=.getBestPairsGenes(cl,fit,pairMat,contrastAdj=contrastAdj,...))
...
.getBestDendroGenes<-function(cl,fit,dendro,contrastAdj,...){
	####
	...
	return(.testContrasts(fit=fit,contrastNames=x, contrastAdj=contrastAdj,...))
...
.getBestPairsGenes<-function(cl,fit,pairMat,contrastAdj,...){
	## basic limma design
...
.testContrasts<-function(fit,contrastNames,contrastAdj,...){
	...
	if(contrastAdj=="AfterF" & p.value<1){
		...
		topsF<-limma::topTable(fitF,genelist=rownames(fit$coef),number=length(rownames(fit$coef)),adjust.method="BH")
