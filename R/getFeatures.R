...
.getBestFGenes<-function(fit, DEMethod,...){
  ...
  if(DEMethod%in% c("limma","limma-voom")){
    ...
    ### Put it into the same form as limma
...
.testContrasts<-function(contr.matrix, contrastNames=NULL, fit,fitF,contrastAdj, DEMethod,weights = NULL, ...){
  ...
  getRaw<-function(ii){
    if(DEMethod%in%c("limma","limma-voom")){
      ...
      #Kevin, does this really have to be done separately for each contrast? That doesn't make sense... 
      #Actually, that's the only way I found to get the top genes for each contrast separately. 
      #Giving the whole contrast matrix returns only one p-value per gene and not one per gene and per contrast.
      # nGenes is the number provided in "..." so the table is going to return the top nGenes genes for each node.
