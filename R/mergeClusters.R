...
#' mergeMethod="adjP", use.edge.length=FALSE,
#' leafType="clusters",labelType="name")
#'
...
setMethod(f = "mergeClusters",
  ...
  if(plot){
    ...
    if(!is.null(dendroSamples)) .plotDendro(dendroSamples,leafType="samples",mergeOutput=out,mergePlotType=plotInfo,mergeMethod=mergeMethod,cl=cl,labelType="name",outbranch=any(cl<0),...)
    else .plotDendro(dendro,leafType="clusters",mergeOutput=out,mergePlotType=plotInfo,mergeMethod=mergeMethod,cl=clMat,labelType="name",...)
...
###Note, plot=FALSE, and then manually call .plotDendro afterwards to allow for passage of colors, etc.
  ...
  if(plot){
    ...
    if(labelType=="id") leg[,"name"]<-leg[,"clusterIds"]
  	...
  	do.call(".plotDendro",c(list(dendro=dend,leafType=leafType,mergeOutput=outlist,mergePlotType=plotInfo,mergeMethod=mergeMethod,cl=cl,clusterLegendMat=leg,labelType=label,outbranch=outbranch,removeOutbranch=outbranch),plotArgs))
  }
