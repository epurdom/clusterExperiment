...
#'   \code{matrix}.
#' @details \code{clusterSingle} is an 'expert-oriented' function, intended to be used when a user wants to run a single clustering and/or have a great deal of control over the clustering parameters. Most users will find \code{\link{clusterMany}} more relevant. Unlike \code{\link{clusterMany}} most of the relevant arguments for clustering are passed to the relevant steps via the arguments \code{clusterDArgs}, \code{subsampleArgs}, and \code{seqArgs}. These arguments should be named lists with parameters that match their corresponding functions: \code{\link{clusterD}},\code{\link{subsampleClustering}}, and \code{\link{seqCluster}}. 
