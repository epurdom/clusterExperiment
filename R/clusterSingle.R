...
#'   \code{\link{subsampleClustering}}. If TRUE, clustering in clusterD step is
#'   done on the co-occurance between clusterings in the subsampled clustering
#'   results.  If FALSE, the clusterD step will be run directly on
#'   \code{x}/\code{diss}
...
#' @param clusterDArgs list of arguments to be passed for the clusterD step, see
#'   help pages of \code{\link{clusterD}}.
...
#'   relevant steps via the arguments \code{clusterDArgs}, \code{subsampleArgs},
#'   and \code{seqArgs}. These arguments should be \emph{named} lists with
#'   parameters that match the corresponding functions:
#'   \code{\link{clusterD}},\code{\link{subsampleClustering}}, and
...
#'   \code{clusterDArgs}: }{The choice of \code{subsample=TRUE} also controls
#'   what algorithm type of clustering functions can be used in the clusterD
...
#'   values) and is passed to \code{diss} of \code{\link{clusterD}}. For this
#'   reason, the \code{ClusterFunction} object given to \code{\link{clusterD}}
#'   via the argument \code{clusterDArgs} must take input of the form of a
...
#'   \code{clusterDArgs} must define a \code{ClusterFunction} object with
...
#'   \code{\link{seqCluster}}).} \item{Setting \code{k} for clusterD step: }{If
#'   \code{sequential=TRUE} then the user should not set \code{k} in the
#'   \code{clusterArgs} argument of \code{clusterDArgs} because it must be set
...
#'   \code{subsample=TRUE}, then the \code{k} in the clustering of clusterD step
...
#'   clusterD step is reasonable. } \item{Setting \code{findBestK} in
#'   \code{clusterDArgs}: }{If \code{sequential=TRUE} and
#'   \code{subsample=FALSE}, the user should not set 'findBestK=TRUE' in
#'   \code{clusterDArgs}. This is because in this case the sequential method
...
#'   'findBestK=FALSE' via \code{clusterDArgs} will function as expected
#'   (assuming the \code{clusterFunction} argument passed to \code{clusterDArgs}
...
#'   that same \code{k} will be used for clusterD step that clusters the
...
#'   and \code{\link{clusterD}},\code{\link{subsampleClustering}}, and
...
#'     subsample=FALSE, sequential=FALSE, clusterDArgs=list(clusterFunction="pam",
