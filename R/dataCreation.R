...
#' @format Three objects are loaded, two data frame(s) of simulated data each with 300 samples/columns and 153 variables/rows, and a vector of length 300 with the true cluster assignments.
#' @details \code{simData} is simulated normal data of 300 simulated observations with 51 relevant variables and the rest of the variables being noise, with observations being in one of 3 groups. \code{simCount} is simulated count data of the same dimension. \code{trueCluster} gives the true cluster identifications of the samples. The true clusters are each of size 100 and are in order in the columns of the data.frames. 
#' @keywords data
#' @examples 
