...
#' if(length(y[y>0]) != 6) stop("rsecFluidigm object has changed -- different # of mergeClusters")
#' adjPValues<-c(0.049794879, 0.007356062, 0.008204838, 0.013156033, 0.009336540, 0.007497524, 0.033526666)
#' if(nrow(rsecFluidigm@merge_nodeProp)!=length(adjPValues)) stop("rsecFluidigm object has changed -- makeDendrogram")
#' if(!all.equal(adjPValues,rsecFluidigm@merge_nodeProp[,"adjP"])) stop("rsecFluidigm object has changed -- different percentages")	
