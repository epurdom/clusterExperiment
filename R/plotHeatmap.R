...
#   definition = function(x,clusterings,
#       clusterOn=c("transformed","PCA","mostVar","dendrogram","original"),
#       visualizeOn=c("original","transformed"),
#       isCount=FALSE,transData=NULL,
#       metaData=NULL,whMetaDataCont=NULL,
#       clusterSamples=TRUE,showSampleNames=FALSE, 
#       clusterFeatures=TRUE,centerAndScaleFeatures=FALSE,whFeatures=c("all","mostVar"),nFeatures=NA,
#       nFeatures=500,showFeatureNames=FALSE,
...
#   origX<-x
#   if("transformed" %in% c(clusterOn,visualizeOn)) transX<-.transData(x,transFun=transFun,isCount=isCount,dimReduce="none")
#   
...
#   if(!is.matrix(clusterings) || !is.data.frame(clusterings)){
#     if(is.vector(clusterings)) 
#   }
