...
#'
#' @details If \code{annCol} contains a column of continuous data, whAnnotCont should give the index of the column(s); otherwise the annotation data for those columns will be forced into a non-sensical factor (with nlevels equal the the number of samples). 
#'
...
#'
#' @details If both \code{annotData} and \code{clusterings} are provided, \code{clusterings} will be shown closest to data (i.e. on bottom) and \code{annotData} on top. A user that wishes greater control should simply combine the two independently and give the combined matrix in annotData.
...
setMethod(
    f = "plotHeatmap",
    signature = signature(annotData = "SummarizedExperiment"),
    definition = function(annotData, 
                          visualize=c("original","transformed","centeredAndScaled"),
                          orderSamples=c("hclust"), #can be indiecs too
                          whichFeatures=c("mostVar","all","PCA"), #can be indices too
                          nFeatures=NULL, metaData=NULL, whMetaCont=NULL,
                          colorScale=if(centerAndScaleFeatures) seqPal3 else seqPal5
    ){	
})
setMethod(
  f = "plotHeatmap",
  signature = signature(annotData = "ClusterExperiment"),
  definition = function(annotData, 
                        visualize=c("original","transformed","centeredAndScaled"),
                        orderSamples=c("dendrogramValue","hclust","orderSamplesValue"), #can be indiecs too
                        whichFeatures=c("mostVar","all","significant","PCA"), #can be indices too
                        nFeatures=NULL, metaData=NULL, whMetaCont=NULL,
                        colorScale=if(centerAndScaleFeatures) seqPal3 else seqPal5
  ){	
    visualize<-match.arg(visualize)
    
    ##Parse the choices for orderSamples and whichFeatures
    ...
    if(is.character(whichFeatures)){
        if(whichFeatures=="significant") stop("Sorry, 'significant' option for whichFeatures is not yet implemented.")
        dimReduce=switch(whichFeatures,
                         "mostVar"="mostVar",
                        "PCA"="PCA",
                        "all"="none")
        if(is.null(nFeatures)) nFeatures<-switch(whichFeatures,"mostVar"=500,"all"=NROW(annotData),"PCA"=50)
        transObj<-.transData(transFun = transformation(x), nPCADims=nFeatures,nVarDims = nFeatures,dimReduce = dimReduce)$whMostVar
    }
    ##Get heatData 
    if(c("transformed","centeredAndScaled") %in% c(visualize)){ 
        heatData<-transform(x,transFun=transformation(annotData),dimReduce="none")  
    } 
    else heatData<-assay(x)
    
    if(clusterOn=="mostVar"){
        clusterSamplesData<-transform(x,transFun=transFun,isCount=isCount,dimReduce="PCA",nPCADims=nFeatures)
    }
    ...
    if(is.character(orderSamples)){
        if(orderSamples=="orderSamplesValues"){
            orderSamples<-orderSamples(annotData)
            clusterSamples<-FALSE
        }
        if(orderSamples=="dendrogram"){
            if(length(dendrogram(annotData))==0){
                #call clusterHclust on primary cluster
            }
            else{
                clusterSamplesData<-dendrogram(annotData)
            }
        }
...
#' @param annotData A matrix of additional annotData to show with the heatmap. Unless indicated by \code{whAnnotCont}, \code{annotData} will be converted into factors.  ``-1'' indicates the sample was not assigned to a cluster and
#' gets color `unassignedColor' and '-2' gets the color 'missingColor'
#' @param whAnnotCont Which of the annotData columns are continuous and should not be converted to counts. NULL indicates no additional annotData.
...
setMethod(
    f = "plotHeatmap",
    signature = signature(annotData = "matrix"),
    definition = function(annotData, heatData,
        ...
        if(!is.matrix(annotData) | !is.data.frame(annotData)){ stop("annotData must be a either a matrix or a data.frame") 
        }
        if(NCOL(heatData) != NROW(annotData)) stop("annotData must have same number of rows as columns of heatData")	
        if(!is.null(annotData)){
            ###Make annotData explicitly factors, except for whAnnotCont 
            ###(not sure why this simpler code doesn't give back data.frame with factors: annCol<-apply(annCol,2,function(x){factor(x)}))
            
            tmpDf<-do.call("data.frame",lapply(1:ncol(annotData),function(ii){factor(annotData[,ii])}))
            names(tmpDf)<-names(annotData)
            if(!is.null(whAnnotCont)) tmpDf[,whAnnotCont]<-annotData[,whAnnotCont]
            annCol<-tmpDf
            ...
            if(is.null(clusterColors)){ #assign default colors
                if(is.null(whAnnCont) || length(whAnnCont)<ncol(annCol)){ #if not all continuous
                    ...
                    if(!is.null(whAnnotCont)) tmpDf<- annCol[,-whAnnCont,drop=FALSE] else tmpDf<-annCol
        ...
        # put into aheatmap
        #############
        breaks<-setBreaks(setBreaks,heatData)
        ...
        invisible(list(aheatmapOut=out,annotData=annCol,clusterColors=clusterColors,breaks=breaks))
    }
)
